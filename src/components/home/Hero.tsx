...
export const Hero = () => {
  ...
  return (
    <section className="relative min-h-[75vh] md:min-h-[90vh] overflow-hidden pt-8 md:pt-16">
      ...
      <AnimatePresence>
        {isMobileMenuOpen && (
          <motion.div
            ...
            className="fixed top-0 left-0 w-4/5 h-screen bg-brand-navy/95 backdrop-blur-lg z-50 shadow-xl border-r border-brand-gold/30 flex flex-col"
            role="dialog"
            aria-modal="true"
          >
      ...
      {/* Background image with the same gradient overlay */}
      <motion.div
      ...
      {/* MOBILE ONLY: Quick Access toggle + panel */}
      <motion.div
        initial={{ opacity: 0, y: 20 }}
        animate={isVisible ? { opacity: 1, y: 0 } : {}}
      ...
      >
        <div className="flex justify-center">
          <button
            ...
            className="rounded-md border border-white/20 bg-brand-navy/70 text-white backdrop-blur px-4 py-2 text-sm shadow-md hover:bg-white/10 focus:outline-none focus-visible:ring focus-visible:ring-brand-gold/60"
            aria-expanded={isMenuOpen}
            aria-controls="mobile-quick-links"
          >
            {isMenuOpen ? "Hide Quick Links" : "Show Quick Links"}
          </button>
        </div>
